...
void l2_prefetcher_operate(int cpu_num, unsigned long long int addr, unsigned long long int ip, int cache_hit)
{
  ...
  for(i=1; i<=16; i++)
    {
	  ...
	  int check_index3 = page_offset - 2 * i - 1;
      ...
      if(ampm_pages[page_index].pf_map[pf_index] == 1)
	{
      ...
      if(((ampm_pages[page_index].access_map[check_index1]==1) && (ampm_pages[page_index].access_map[check_index2]==1))
		  || (check_index3 >= 0 && ((ampm_pages[page_index].access_map[check_index1] == 1) && 
		  (ampm_pages[page_index].access_map[check_index3] == 1))))
	{
  ...
  for(i=1; i<=16; i++)
      ...
      if(ampm_pages[page_index].pf_map[pf_index] == 1)
	{
      ...
      if(((ampm_pages[page_index].access_map[check_index1]==1) && (ampm_pages[page_index].access_map[check_index2]==1))
		  || (check_index3 <= 63 && ((ampm_pages[page_index].access_map[check_index1] == 1) 
			  && (ampm_pages[page_index].access_map[check_index3] == 1))))
	{
	  // we found the stride repeated twice, so issue a prefetch
